...
import { TaxApi, TaxZoneSummary } from '@commerce/api';
...
@Component({
  ...
  template: `
    <cmr-breadcrumbs [items]="[
      ...
      { label: ('tax.zones.title' | translate) }
    ]" />
    <header class="page-header">
      <h1>{{ 'tax.zones.title' | translate }}</h1>
      @if (permissions.hasPermission('Tax.Manage')) {
        <a routerLink="/tax/zones/new" class="btn">{{ 'action.create' | translate }}</a>
      }
    </header>
      ...
      @default {
        <div class="table-wrap">
          <table>
            <thead>
              <tr>
                <th>{{ 'tax.zones.name' | translate }}</th>
                <th>{{ 'tax.zones.systemName' | translate }}</th>
                <th>{{ 'tax.zones.isDefault' | translate }}</th>
            ...
            <tbody>
              @for (item of items; track item.id) {
                <tr>
                  ...
                  <td>{{ item.isDefault ? ('tax.yes' | translate) : ('tax.no' | translate) }}</td>
                  <td>{{ item.displayOrder }}</td>
                  <td>{{ item.isActive ? ('tax.active' | translate) : ('tax.inactive' | translate) }}</td>
                  <td class="actions">
                    @if (permissions.hasPermission('Tax.Manage')) {
                      <a [routerLink]="['/tax/zones', item.id]">{{ 'action.edit' | translate }}</a>
    ...
    <cmr-confirm-dialog
      ...
      [title]="'tax.zones.deleteTitle' | translate"
      [message]="('tax.zones.deleteMessage' | translate) + ' ' + (deleteTarget?.name ?? '')"
...
export class ZoneListPageComponent implements OnInit {
  ...
  errorMessage = '';
  items: TaxZoneSummary[] = [];
  deleteTarget: TaxZoneSummary | null = null;
  ...
  async load(): Promise<void> {
    ...
    this.errorMessage = '';
    try {
      this.items = await firstValueFrom(this.taxApi.listZones());
      this.state = this.items.length === 0 ? 'empty' : 'success';
    } catch (error) {
      this.errorMessage = error instanceof ApiClientError ? error.message : 'Failed to load tax zones.';
      this.state = 'error';
    }
  }

  confirmDelete(item: TaxZoneSummary): void {
  ...
  async deleteConfirmed(): Promise<void> {
    ...
    try {
      await firstValueFrom(this.taxApi.deleteZone(this.deleteTarget.id));
