...
import { ShippingApi, ShippingZoneSummary } from '@commerce/api';
...
@Component({
  ...
  template: `
    <cmr-breadcrumbs [items]="[
      ...
      { label: ('shipping.zones.title' | translate) }
    ]" />
    <header class="page-header">
      <h1>{{ 'shipping.zones.title' | translate }}</h1>
      @if (permissions.hasPermission('Shipping.Manage')) {
        <a routerLink="/shipping/zones/new" class="btn">{{ 'action.create' | translate }}</a>
      }
    </header>
      ...
      @default {
        <div class="table-wrap">
          <table>
            <thead>
              <tr>
                <th>{{ 'shipping.zones.name' | translate }}</th>
                <th>{{ 'shipping.zones.systemName' | translate }}</th>
                <th>{{ 'shipping.zones.isDefault' | translate }}</th>
            ...
            <tbody>
              @for (item of items; track item.id) {
                <tr>
                  ...
                  <td>{{ item.isDefault ? ('shipping.yes' | translate) : ('shipping.no' | translate) }}</td>
                  <td>{{ item.displayOrder }}</td>
                  <td>{{ item.isActive ? ('shipping.active' | translate) : ('shipping.inactive' | translate) }}</td>
                  <td class="actions">
                    @if (permissions.hasPermission('Shipping.Manage')) {
                      <a [routerLink]="['/shipping/zones', item.id]">{{ 'action.edit' | translate }}</a>
    ...
    <cmr-confirm-dialog
      ...
      [title]="'shipping.zones.deleteTitle' | translate"
      [message]="('shipping.zones.deleteMessage' | translate) + ' ' + (deleteTarget?.name ?? '')"
...
export class ZoneListPageComponent implements OnInit {
  ...
  errorMessage = '';
  items: ShippingZoneSummary[] = [];
  deleteTarget: ShippingZoneSummary | null = null;
  ...
  async load(): Promise<void> {
    ...
    this.errorMessage = '';
    try {
      this.items = await firstValueFrom(this.shippingApi.listZones());
      this.state = this.items.length === 0 ? 'empty' : 'success';
    } catch (error) {
      this.errorMessage = error instanceof ApiClientError ? error.message : 'Failed to load shipping zones.';
      this.state = 'error';
    }
  }

  confirmDelete(item: ShippingZoneSummary): void {
  ...
  async deleteConfirmed(): Promise<void> {
    ...
    try {
      await firstValueFrom(this.shippingApi.deleteZone(this.deleteTarget.id));
